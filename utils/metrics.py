...
def get_ntrainparams(model):  # calculate model size
...
class IoU(Metric):
    ...
    def value(self):
        ...
        metrics = {'IoU': iou, 'mIoU': np.nanmean(iou) * 100, 'Acc': acc, 'OA': all_acc * 100, 'Precision': prc, 'Recall': rcl}
        return metrics
...
def one_hot(index, classes, device):
    ...
    mask = torch.Tensor(size).fill_(0).to(device)  # (P, C)
...
class FocalLoss(nn.Module):
    ...
    def __init__(self, gamma=2, eps=1e-7, size_average=True, one_hot=True, ignore=None, device='cpu'):
    ...
    def forward(self, input, target):
        ...
        if self.one_hot:
            target = one_hot(target, C, self.device)  # (P, C)
